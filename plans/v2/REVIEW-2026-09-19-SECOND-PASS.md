...
Fetched `origin` and fast-forwarded the current branch to `origin/main` at `3310e2f14` (merge of PR #9). Existing local edits were preserved. During review, another editor committed the intranet revisions, ending at `2e87cb292` (`docs(plans): adapt plans to an intranet-only, no-internet deployment`). This report was then checked against that completed revision, including its expanded mail-router design. Source links open the working files. The review itself is the only file modified by this pass; no plan corrections or implementation changes were made here.
...
### R2-7 — P1: the proposed IMAP routing callback cannot access the message it must forward

[Plan 20 §4.11](/Users/adamstavely/Documents/GitHub/quackback/plans/v2/20-control-tower.md) now correctly recognizes that [the upstream IMAP scheduler](/Users/adamstavely/Documents/GitHub/quackback/apps/web/src/lib/server/domains/conversation/conversation.email-imap-queue.ts:47) refuses pooled tenancy and specifies a dedicated fleet router. The final intranet revision adds a singleton lock, per-slug delivery records, envelope-recipient headers, retry handling, and a dead-message policy. The previous configuration-only assumption and SES/Lambda implementation path have been corrected.

A concrete interface mismatch remains: the proposed router calls upstream `pollOnce` with a callback that extracts envelope headers and POSTs the original raw MIME. However, `pollOnce` passes only **parsed email** to its callback. The normalized object does not retain the raw message or arbitrary delivery headers. `fetchUnseen` exposes the raw message, but `pollOnce` discards that representation before invoking the callback: [conversation.email-imap.ts:34](/Users/adamstavely/Documents/GitHub/quackback/apps/web/src/lib/server/domains/conversation/conversation.email-imap.ts:34), line 78.

**Required change:** Specify a fork raw-message polling loop over `createImapClient`/`fetchUnseen`, or a narrowly extended upstream adapter that passes the original raw message and trusted envelope metadata. Adjust the claimed reuse/seam inventory accordingly. Require the internal mail server to strip untrusted copies of the selected routing headers before stamping its own. Clarify how accepted-but-unrecorded deliveries are deduplicated at the receiving tenant: the router's delivery record alone cannot prevent a repeat after that crash window. Define handling of permanent HTTP 4xx outcomes as well as timeout/5xx.

**Acceptance test:** Deliver cold mail and replies to two tenants, include BCC/multiple recipients and forged delivery headers, retry after accepted delivery but before the router records success, and verify no unintended tenant delivery or duplicate visible messages. Exercise raw MIME preservation through the exact planned adapter.
